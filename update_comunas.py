#!/usr/bin/env python3
"""
Script para actualizar todas las comunas con:
1. 4 tarjetas de servicios destacados (Cambio de Aceite, Frenos, Electricidad, Escáner)
2. Sección de Vehículos con modal lightbox (fotos miniatura que se abren en grande)
3. Corregir posición de botones flotantes en index.html
"""
...
INDEX_FILE = "/home/z/my-project/Globalpro/index.html"

# Mapping: comuna slug -> comuna name
COMUNA_NAMES = {
    "alhue": "Alhué",
    "buin": "Buín",
...
# Vehicle data with thumbnail and full-size images
VEHICLES = [
...
def generate_featured_services_section(comuna_name, comuna_slug):
    """Generate the 4 featured service cards HTML for a comuna."""
  ...
  <div class="container">
    <div class="text-center mb-5">
      <h2 class="fw-bold" style="font-size: 2.2rem; color: #FFC107;">
        <i class="fas fa-star me-2"></i>Servicios Más Solicitados en {comuna_name}
      </h2>
      <p style="color: #ccc; max-width: 700px; margin: 0 auto;">Los servicios que más necesitan los vecinos de {comuna_name}. Agenda rápido por WhatsApp.</p>
    </div>
    <div class="row g-4 justify-content-center">
      <div class="col-lg-3 col-md-6">
        <div class="p-4 rounded text-center h-100" style="background: rgba(255,255,255,0.07); border: 2px solid #FFC107; border-radius: 16px !important; transition: transform 0.3s, box-shadow 0.3s;" onmouseover="this.style.transform='translateY(-8px)';this.style.boxShadow='0 10px 30px rgba(255,193,7,0.3)'" onmouseout="this.style.transform='translateY(0)';this.style.boxShadow='none'">
          ...
          <p style="font-size:0.9rem; color:#ccc; line-height:1.7;">Aceites sintéticos y semi-sintéticos premium. Incluye filtro y revisión de niveles. A domicilio en {comuna_name}.</p>
          <a href="{wa_base}?text=Hola%20necesito%20un%20cambio%20de%20aceite%20a%20domicilio%20en%20{comuna_name.replace(' ', '%20')}" class="btn btn-sm fw-bold mt-2" target="_blank" style="background:#FFC107; color:#000; border-radius:50px; padding:8px 20px;">
      ...
      <div class="col-lg-3 col-md-6">
        <div class="p-4 rounded text-center h-100" style="background: rgba(255,255,255,0.07); border: 2px solid #D32F2F; border-radius: 16px !important; transition: transform 0.3s, box-shadow 0.3s;" onmouseover="this.style.transform='translateY(-8px)';this.style.boxShadow='0 10px 30px rgba(211,47,47,0.3)'" onmouseout="this.style.transform='translateY(0)';this.style.boxShadow='none'">
          ...
          <p style="font-size:0.9rem; color:#ccc; line-height:1.7;">Pastillas, discos, líquido y ABS. Medición digital del espesor. Tu seguridad es primero en {comuna_name}.</p>
          <a href="{wa_base}?text=Hola%20necesito%20reparacion%20de%20frenos%20a%20domicilio%20en%20{comuna_name.replace(' ', '%20')}" class="btn btn-sm fw-bold mt-2" target="_blank" style="background:#D32F2F; color:#fff; border-radius:50px; padding:8px 20px;">
            <i class="fab fa-whatsapp me-1"></i> Cotizar
          </a>
        </div>
      </div>
      <div class="col-lg-3 col-md-6">
        <div class="p-4 rounded text-center h-100" style="background: rgba(255,255,255,0.07); border: 2px solid #2196F3; border-radius: 16px !important; transition: transform 0.3s, box-shadow 0.3s;" onmouseover="this.style.transform='translateY(-8px)';this.style.boxShadow='0 10px 30px rgba(33,150,243,0.3)'" onmouseout="this.style.transform='translateY(0)';this.style.boxShadow='none'">
          ...
          <p style="font-size:0.9rem; color:#ccc; line-height:1.7;">Diagnóstico eléctrico completo. Batería, alternador, motor de arranque y cableado a domicilio en {comuna_name}.</p>
          <a href="{wa_base}?text=Hola%20necesito%20electricidad%20automotriz%20a%20domicilio%20en%20{comuna_name.replace(' ', '%20')}" class="btn btn-sm fw-bold mt-2" target="_blank" style="background:#2196F3; color:#fff; border-radius:50px; padding:8px 20px;">
            <i class="fab fa-whatsapp me-1"></i> Cotizar
          </a>
        </div>
      </div>
      <div class="col-lg-3 col-md-6">
        <div class="p-4 rounded text-center h-100" style="background: rgba(255,255,255,0.07); border: 2px solid #4CAF50; border-radius: 16px !important; transition: transform 0.3s, box-shadow 0.3s;" onmouseover="this.style.transform='translateY(-8px)';this.style.boxShadow='0 10px 30px rgba(76,175,80,0.3)'" onmouseout="this.style.transform='translateY(0)';this.style.boxShadow='none'">
          ...
          <h3 style="font-weight:700; font-size:1.2rem; color:#4CAF50; margin-bottom:10px;">Escáner</h3>
          <p style="font-size:0.9rem; color:#ccc; line-height:1.7;">Diagnóstico computarizado sin adivinanzas. Interpretación de códigos y soluciones reales en {comuna_name}.</p>
          <a href="{wa_base}?text=Hola%20necesito%20scanner%20automotriz%20a%20domicilio%20en%20{comuna_name.replace(' ', '%20')}" class="btn btn-sm fw-bold mt-2" target="_blank" style="background:#4CAF50; color:#fff; border-radius:50px; padding:8px 20px;">
            <i class="fab fa-whatsapp me-1"></i> Cotizar
          </a>
        </div>
      </div>
...
def generate_vehicles_section(comuna_name, comuna_slug):
    ...
    for i, v in enumerate(VEHICLES):
        ...
        img_path = f"../images/wp-content/uploads/2024/10/{v['thumb']}" if not v['thumb'].startswith('Fiat') and not v['thumb'].startswith('33678') and not v['thumb'].startswith('pngegg') else f"../images/wp-content/uploads/2024/{'08' if v['thumb'].startswith('Fiat') or v['thumb'].startswith('pngegg') else '10'}/{v['thumb']}"
        ...
        <div class="col-4 col-md-3 col-lg-2 veh-thumb-col" style="display:{display};">
          <div class="veh-thumb" onclick="openVehicleLightbox({i})">
            <img src="{img_path}" alt="{v['alt']} en {comuna_name}" loading="lazy">
    ...
    for i, v in enumerate(VEHICLES):
        img_dir = '10'
        if v['full'].startswith('Fiat') or v['full'].startswith('pngegg'):
            img_dir = '08'
        full_path = f"../images/wp-content/uploads/2024/{img_dir}/{v['full']}"
  ...
  <div class="container">
    <div class="text-center mb-4">
      <h2 class="fw-bold" style="color:#a80000; font-size:2rem;">
        <i class="fas fa-car me-2"></i>Vehículos que Atendemos en {comuna_name}
      </h2>
      <p style="color:#666; max-width:600px; margin:0 auto;">Selecciona tu vehículo para ver los servicios disponibles. Haz clic en la foto para ampliarla.</p>
    </div>
    <div class="row g-3 justify-content-center">
      {''.join(vehicle_cards)}
    </div>
    <div class="text-center mt-4" id="veh-load-more-container-{comuna_slug}">
      <button class="btn btn-outline-primary fw-bold" onclick="loadMoreVehicles('{comuna_slug}')">
        <i class="fas fa-plus-circle me-1"></i> Ver Más Vehículos
      </button>
    </div>
    <div class="text-center mt-4">
      <a href="https://wa.me/56939026185?text=Hola%20necesito%20servicio%20para%20mi%20vehiculo%20en%20{comuna_name.replace(' ', '%20')}" class="btn btn-primary btn-lg fw-bold" target="_blank">
        <i class="fab fa-whatsapp me-2"></i> Cotizar para mi Vehículo
      </a>
    </div>
  </div>
</section>
...
<div id="vehicleLightbox" style="display:none; position:fixed; top:0; left:0; right:0; bottom:0; background:rgba(0,0,0,0.92); z-index:10001; align-items:center; justify-content:center; padding:15px;" onclick="if(event.target===this)closeVehicleLightbox()">
  ...
  <div style="text-align:center; max-width:90vw; max-height:85vh;">
    <img id="lightboxImg" src="" alt="Vehículo" style="max-width:90vw; max-height:75vh; object-fit:contain; border-radius:12px; box-shadow:0 10px 40px rgba(0,0,0,0.5);">
...
def get_comuna_slug_from_filename(filepath):
    """Extract comuna slug from filename."""
    basename = os.path.basename(filepath)
    return basename.replace('.html', '')


def update_comuna_file(filepath):
    """Update a single comuna HTML file with services + vehicles section."""
    slug = get_comuna_slug_from_filename(filepath)
    ...
    services_html = generate_featured_services_section(comuna_name, slug)
    vehicles_html = generate_vehicles_section(comuna_name, slug)

    # Insert services section AFTER the SEO section (before "RESEÑAS" or "CONECTAMOS")
    # Strategy: Insert before the RESEÑAS section or CONECTAMOS section
    insertion_points = [
        '<!-- RESEÑAS -->',
        '<!-- CONECTAMOS MARQUEE -->',
        '<section id="resenas"',
    ...
    inserted_services = False
    for point in insertion_points:
        idx = content.find(point)
        if idx != -1:
            # Find the actual start of the section (go back to find <!--)
            comment_idx = content.rfind('<!--', 0, idx)
            if comment_idx != -1 and idx - comment_idx < 100:
                insert_at = comment_idx
            else:
                insert_at = idx
            content = content[:insert_at] + services_html + '\n' + content[insert_at:]
            inserted_services = True
            break

    if not inserted_services:
        # Fallback: insert after the SEO section close
        seo_end = content.find('</section>', content.find('seo-section'))
        if seo_end != -1:
            insert_at = seo_end + len('</section>')
            content = content[:insert_at] + '\n' + services_html + content[insert_at:]
            inserted_services = True

    # Insert vehicles section BEFORE the CONTACTO section
    contact_points = [
        '<!-- CONTACTO -->',
        '<section class="py-5" style="background: linear-gradient(135deg, #a80000',
        '<!-- FOOTER -->',
    ]

    inserted_vehicles = False
    for point in contact_points:
        idx = content.find(point)
        if idx != -1:
            # Find the comment before the section
            comment_idx = content.rfind('<!--', 0, idx)
            if comment_idx != -1 and idx - comment_idx < 100:
                insert_at = comment_idx
            else:
                insert_at = idx
            content = content[:insert_at] + vehicles_html + '\n' + content[insert_at:]
            inserted_vehicles = True
            break

    if not inserted_vehicles:
        # Fallback: insert before footer
        footer_idx = content.find('<footer')
        if footer_idx != -1:
            content = content[:footer_idx] + vehicles_html + '\n' + content[footer_idx:]
            inserted_vehicles = True

    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(content)

    return inserted_services or inserted_vehicles


def fix_index_buttons():
    """Fix the 3 floating buttons in index.html - lower them 4cm (~150px) from the navbar."""
    with open(INDEX_FILE, 'r', encoding='utf-8') as f:
        content = f.read()

    # Change top: 85px to top: 235px (85 + 150 = 235, which is ~4cm lower)
    # Also need to update the media query if there is one
    content = content.replace(
        'top: 85px;',
        'top: 235px;'
    )

    # Check for responsive version
    if '#floating-top-buttons' in content and 'top: 235px' in content:
        # Also update any media query for the floating buttons
        content = content.replace(
    ...
    print("Fixed index.html floating buttons: moved from top:85px to top:235px")


def main():
    print("=" * 60)
    print("ACTUALIZACIÓN MASIVA DE COMUNAS")
    print("=" * 60)

    # Get all comuna HTML files
    comuna_files = sorted(glob.glob(os.path.join(COMUNAS_DIR, "*.html")))
    print(f"\nFound {len(comuna_files)} comuna files")
    ...
    for filepath in comuna_files:
        slug = get_comuna_slug_from_filename(filepath)
        comuna_name = COMUNA_NAMES.get(slug, slug)
        print(f"\nProcessing: {comuna_name} ({slug})...")

        try:
            result = update_comuna_file(filepath)
            if result:
                updated += 1
                print(f"  ✓ UPDATED {comuna_name}")
            else:
                skipped += 1
    ...
    print("\n" + "=" * 60)
    print("Fixing index.html floating buttons...")
    fix_index_buttons()

    print("\n" + "=" * 60)
    print(f"RESUMEN:")
    print(f"  Comunas actualizadas: {updated}")
    print(f"  Comunas saltadas: {skipped}")
    print(f"  Errores: {errors}")
    print(f"  Total archivos: {len(comuna_files)}")
    print("=" * 60)
